...
export class OCU_BTC_USD__peekResult {
  value0: BigInt;
  value1: boolean;
...
export class OCU_BTC_USD extends ethereum.SmartContract {
  static bind(address: Address): OCU_BTC_USD {
    return new OCU_BTC_USD("OCU_BTC_USD", address);
  }
  ...
  peek(): OCU_BTC_USD__peekResult {
    let result = super.call("peek", "peek():(uint256,bool)", []);

    return new OCU_BTC_USD__peekResult(
      result[0].toBigInt(),
      result[1].toBoolean()
    );
  }

  try_peek(): ethereum.CallResult<OCU_BTC_USD__peekResult> {
    ...
    return ethereum.CallResult.fromValue(
      new OCU_BTC_USD__peekResult(value[0].toBigInt(), value[1].toBoolean())
    );
  }
